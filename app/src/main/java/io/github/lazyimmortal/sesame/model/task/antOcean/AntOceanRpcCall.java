...
public class AntOceanRpcCall {
    ...
    public static String queryOceanStatus() {
        return ApplicationHook.requestString("alipay.antocean.ocean.h5.queryOceanStatus", "[{\"source" +
    ...
    public static String ipOpenSurprise() {
        return ApplicationHook.requestString("alipay.antocean.ocean.h5.ipOpenSurprise", "[{\"source" +
                "\":\"chInfo_ch_appcenter__chsub_9patch\",\"uniqueId\":\"" + getUniqueId() + "\"}]");
    }

    public static String collectReplicaAsset() {
        return ApplicationHook.requestString("alipay.antocean.ocean.h5.collectReplicaAsset", "[{\"replicaCode" +
                "\":\"avatar\",\"source\":\"senlinzuoshangjiao\",\"uniqueId\":\"" + getUniqueId() + "\",\"version" +
                "\":\"" + VERSION + "\"}]");
    }
    ...
    public static String receiveTaskAward(String sceneCode, String taskType) {
        return ApplicationHook.requestString("com.alipay.antiep.receiveTaskAward", "[{\"ignoreLimit\":false," +
                "\"requestType\":\"RPC\",\"sceneCode\":\"" + sceneCode + "\",\"source\":\"ANT_FOREST\"," +
                "\"taskType\":\"" + taskType + "\",\"uniqueId\":\"" + getUniqueId() + "\"}]");
    }
    ...
    public static String finishTask(String sceneCode, String taskType) {
        ...
        return ApplicationHook.requestString("com.alipay.antiep.finishTask",
                "[{\"outBizNo\":\"" + outBizNo + "\"," + "\"requestType\":\"RPC\",\"sceneCode\":\"" + sceneCode +
                        "\",\"source\":\"ANTFOCEAN\"," + "\"taskType\":\"" + taskType + "\",\"uniqueId\":\"" + getUniqueId() + "\"}]");
    }

    public static String queryTaskList() {
        return ApplicationHook.requestString("alipay.antocean.ocean.h5.queryTaskList", "[{\"extend\":{}," +
                "\"fromAct\":\"dynamic_task\",\"sceneCode\":\"ANTOCEAN_TASK\",\"source\":\"ANT_FOREST\"," +
                "\"uniqueId\":\"" + getUniqueId() + "\",\"version\":\"" + VERSION + "\"}]");
    }
    ...
    public static String unLockReplicaPhase(String replicaCode, String replicaPhaseCode) {
        return ApplicationHook.requestString("alipay.antocean.ocean.h5.unLockReplicaPhase",
                "[{\"replicaCode\":\"" + replicaCode + "\",\"replicaPhaseCode\":\"" + replicaPhaseCode + "\",\"source"
                        + "\":\"senlinzuoshangjiao\",\"uniqueId\":\"" + getUniqueId() + "\",\"version\":\"20220707" +
                        "\"}]");
    }
    ...
    public static String queryReplicaTaskList() {
        return ApplicationHook.requestString("alipay.antocean.ocean.h5.queryTaskList",
                "[{\"fromAct\":\"dynamic_task" + "\",\"sceneCode\":\"ANTOCEAN_AVATAR_TASK\"," +
                        "\"source\":\"seaAreaList\",\"uniqueId\":\"" + getUniqueId() + "\",\"version\":\"" + VERSION + "\"}]");
    }

    public static String receiveReplicaTaskAward(String taskType) {
        return ApplicationHook.requestString("com.alipay.antiep.receiveTaskAward", "[{\"ignoreLimit\":\"false\"," +
                "\"requestType\":\"RPC\",\"sceneCode\":\"ANTOCEAN_AVATAR_TASK\",\"source\":\"ANTFOCEAN\"," +
                "\"taskType\":\"" + taskType + "\",\"uniqueId\":\"" + getUniqueId() + "\"}]");
    }

    public static String repairSeaArea() {
        return ApplicationHook.requestString("alipay.antocean.ocean.h5.repairSeaArea", "[{\"source" +
                "\":\"chInfo_ch_appcenter__chsub_9patch\",\"uniqueId\":\"" + getUniqueId() + "\"}]");
    }
    ...
    public static String queryOceanPropList(String propTypeList) {
        String args =
                "[{\"propTypeList\":\"" + propTypeList + "\",\"skipPropId\":false,\"source\":\"ANT_FOREST\"," +
    ...
    public static String querySeaAreaDetailList() {
        return ApplicationHook.requestString("alipay.antocean.ocean.h5.querySeaAreaDetailList", "[{\"seaAreaCode" +
                "\":\"\",\"source\":\"chInfo_ch_appcenter__chsub_9patch\",\"targetUserId\":\"\",\"uniqueId\":\"" + getUniqueId() + "\"}]");
    }

    public static String queryOceanChapterList() {
        return ApplicationHook.requestString("alipay.antocean.ocean.h5.queryOceanChapterList", "[{\"source" +
                "\":\"chInfo_ch_url-https://2021003115672468.h5app.alipay.com/www/atlasOcean.html\",\"uniqueId\":\"" + getUniqueId() + "\"}]");
    }
    ...
    public static String queryMiscInfo() {
        return ApplicationHook.requestString("alipay.antocean.ocean.h5.queryMiscInfo", "[{\"queryBizTypes" +
                "\":[\"HOME_TIPS_REFRESH\"],\"source\":\"chInfo_ch_appcenter__chsub_9patch\",\"uniqueId\":\"" + getUniqueId() + "\"}]");
    }
    ...
    public static String queryFriendPage(String userId) {
        return ApplicationHook.requestString("alipay.antocean.ocean.h5.queryFriendPage",
                "[{\"friendUserId\":\"" + userId + "\",\"interactFlags\":\"T\",\"source\":\"ANT_FOREST\"," +
                        "\"uniqueId\":\"" + getUniqueId() + "\",\"version\":\"" + VERSION + "\"}]");
    }
    ...
    public static String submitAnswer(String answer, String questionId) {
        return ApplicationHook.requestString("com.alipay.reading.game.dada.openDailyAnswer.submitAnswer",
                "[{\"activityId\":\"363\",\"answer\":\"" + answer + "\",\"dadaVersion\":\"1.3.0\"," + "\"outBizId" +
                        "\":\"ANTOCEAN_DATI_PINTU_722_new\",\"questionId\":\"" + questionId + "\",\"version" +
                        "\":\"1\"}]");
    }
    ...
    public static String exchangeProp(int exchangeNum, String propCode, String propType) {
        ...
        return ApplicationHook.requestString("alipay.antocean.ocean.h5.exchangeProp", "[{\"bizNo\":\"" + timestamp +
                "\",\"exchangeNum\":\"" + exchangeNum + "\",\"propCode\":\"" + propCode + "\",\"propType\":\"" + propType + "\",\"source\":\"ANT_FOREST\",\"uniqueId\":\"" + getUniqueId() + "\"}]");
    }
    ...
    public static String queryFishList(int pageNum) {
        return ApplicationHook.requestString("alipay.antocean.ocean.h5.queryFishList", "[{\"combineStatus" +
                "\":\"UNOBTAINED\",\"needSummary\":\"Y\",\"pageNum\":" + pageNum + ",\"targetUserId\":\"\"," +
                "\"uniqueId\":\"" + getUniqueId() + "\"}]");
    }

    public static String usePropByType(String propCode, String propType, JSONArray assetsDetails) {
        String args =
                "[{\"assetsDetails\":" + assetsDetails + ",\"propCode\":\"" + propCode + "\",\"propType\":\"" + propType + "\",\"uniqueId\":\"" + getUniqueId() + "\"}]";
