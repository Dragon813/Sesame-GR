...
public class AntDodo extends ModelTask {
    ...
    public ModelFields getFields() {
        ...
        modelFields.addField(usePropList = new SelectModelField("usePropList", "使用道具 | 道具列表", new LinkedHashSet<>(),
                CustomOption::getAntDodoPropList));
        modelFields.addField(useCollectTimingType = new ChoiceModelField("useCollectTimingType", "抽卡道具 | 使用时机",
                TimingType.EVERY_DAY, TimingType.nickNames));
        modelFields.addField(useUniversalCardBookStatusType = new ChoiceModelField("useUniversalCardBookStatusType",
                "万能卡片 | 图鉴状态类型", BookStatusType.END, BookStatusType.nickNames));
        ...
        modelFields.addField(useUniversalCardBookCollectedhasCollected = new BooleanModelField(
                "useUniversalCardBookCollectedhasCollected", "万能卡片 | 优先兑换未获得状态卡片", false));
        ...
        modelFields.addField(useUniversalCardFantasticLevelType = new ChoiceModelField(
                "useUniversalCardFantasticLevelType", "万能卡片 | 最低等级", FantasticLevelType.MAGIC,
                FantasticLevelType.nickNames));
        modelFields.addField(bookMedal = new BooleanModelField("bookMedal", "图鉴勋章 | 开启", false));
        modelFields.addField(bookMedalOptions = new SelectModelField("bookMedalOptions", "图鉴勋章 | 选项",
                new LinkedHashSet<>(), CustomOption::getAntDodoBookMedalOptions));
        modelFields.addField(collectToFriendType = new ChoiceModelField("collectToFriendType", "帮抽卡片 | 动作",
                CollectToFriendType.NONE, CollectToFriendType.nickNames));
        modelFields.addField(collectToFriendList = new SelectModelField("collectToFriendList", "帮抽卡片 | 好友列表",
                new LinkedHashSet<>(), AlipayUser::getList));
        modelFields.addField(giftToFriend = new BooleanModelField("giftToFriend", "赠送卡片 | 开启", false));
        modelFields.addField(giftToFriendBookStatusType = new ChoiceModelField("giftToFriendBookStatusType", "赠送卡片 | "
                + "图鉴状态类型", BookStatusType.ALL, BookStatusType.nickNames));
        modelFields.addField(giftToFriendBookCollectedStatusType = new ChoiceModelField(
                "giftToFriendBookCollectedStatusType", "赠送卡片 | 图鉴收集状态", BookCollectedStatusType.ALL,
                BookCollectedStatusType.nickNames));
        modelFields.addField(giftToFriendMedalGenerationStatusType = new ChoiceModelField(
                "giftToFriendMedalGenerationStatusType", "赠送卡片 | 勋章合成状态", MedalGenerationStatusType.ALL,
                MedalGenerationStatusType.nickNames));
        modelFields.addField(giftToFriendFantasticLevelType = new ChoiceModelField("giftToFriendFantasticLevelType",
                "赠送卡片 | 最低等级", FantasticLevelType.COMMON, FantasticLevelType.nickNames));
        modelFields.addField(giftToFriendList = new SelectModelField("giftToFriendList", "赠送卡片 | 好友列表",
                new LinkedHashSet<>(), AlipayUser::getList, "会赠送所有满足条件的卡片给已选择的好友"));
        return modelFields;
    }
    ...
    public Boolean check() {
        if (TaskCommon.IS_ENERGY_TIME) {
            Log.forest("任务暂停⏸️神奇物种:当前为仅收能量时间"); return false;
        } return true;
    }
    ...
    public void run() {
        try {
            collect(); taskList(); if (useProp.getValue()) {
                propList();
            ...
            Log.i(TAG, "AntoDodo.start.run err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private long getEndDateTime() {
        try {
            JSONObject jo = new JSONObject(AntDodoRpcCall.homePage()); if (!MessageUtil.checkResultCode(TAG, jo)) {
                return 0;
            } jo = jo.getJSONObject("data"); jo = jo.getJSONObject("animalBook");
            String endDate = jo.getString("endDate") + " 23:59:59"; return Log.timeToStamp(endDate);
        } catch (Throwable t) {
            Log.i(TAG, "getEndDateTime err:"); Log.printStackTrace(TAG, t);
        } return 0;
    }
    ...
    private void collect() {
        if (Status.hasFlagToday("dodo::collect")) {
            ...
            if (MessageUtil.checkResultCode(TAG, jo)) {
                JSONObject data = jo.getJSONObject("data"); if (data.getBoolean("collect")) {
            ...
            Log.i(TAG, "collect err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private void collectAnimalCard() {
        try {
            JSONObject jo = new JSONObject(AntDodoRpcCall.homePage()); if (MessageUtil.checkResultCode(TAG, jo)) {
                JSONObject data = jo.getJSONObject("data"); JSONArray ja = data.getJSONArray("limit"); int index = -1;
                for (int i = 0; i < ja.length(); i++) {
                    jo = ja.getJSONObject(i); if ("DAILY_COLLECT".equals(jo.getString("actionCode"))) {
                        index = i; break;
                    }
                } if (index >= 0) {
                    int leftFreeQuota = jo.getInt("leftFreeQuota"); for (int j = 0; j < leftFreeQuota; j++) {
                        jo = new JSONObject(AntDodoRpcCall.collect()); if (MessageUtil.checkResultCode(TAG, jo)) {
                            data = jo.getJSONObject("data"); JSONObject animal = data.getJSONObject("animal");
                            Log.forest("神奇物种🦕每日抽卡" + getAnimalInfo(animal)); checkAnimalAndGiftToFriend(animal);
                        }
                    }
                }
            }
        } catch (Throwable t) {
            Log.i(TAG, "collectAnimalCard err:"); Log.printStackTrace(TAG, t);
        }
    }

    private void taskList() {
        try {
            JSONObject jo = new JSONObject(AntDodoRpcCall.taskList()); if (!MessageUtil.checkResultCode(TAG, jo)) {
                return;
            } jo = jo.getJSONObject("data"); JSONArray taskGroupInfoList = jo.optJSONArray("taskGroupInfoList");
            ...
            Log.i(TAG, "taskList err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private Boolean finishTask(String sceneCode, String taskType, String taskTitle) {
        try {
            ...
            if (MessageUtil.checkSuccess(TAG, jo)) {
                Log.forest("神奇物种🦕完成[" + taskTitle + "]"); return true;
            }
        } catch (Throwable t) {
    ...
    private void propList() {
        try {
            th:
            do {
                JSONObject jo = new JSONObject(AntDodoRpcCall.propList()); if (!MessageUtil.checkResultCode(TAG, jo)) {
                    break;
                } jo = jo.getJSONObject("data"); JSONArray propList = jo.getJSONArray("propList");
                for (int i = 0; i < propList.length(); i++) {
                    JSONObject prop = propList.getJSONObject(i); String propType = prop.getString("propType");
                    String propGroup = prop.getJSONObject("propConfig").getString("propGroup");
                    JSONArray propIdList = prop.getJSONArray("propIdList"); String propId = propIdList.getString(0);
            ...
            Log.i(TAG, "propList err:"); Log.printStackTrace(TAG, th);
        }
    }
    ...
    private Boolean usePropUniversalCard(String propId, String propType) {
        try {
            boolean hasMore; int pageStart = 0; JSONObject animal = null; do {
                ...
                if (!MessageUtil.checkResultCode(TAG, jo)) {
                    break;
                } jo = jo.getJSONObject("data"); hasMore = jo.getBoolean("hasMore"); pageStart += 9;
            ...
            Log.i(TAG, "usePropUniversalCard err:"); Log.printStackTrace(TAG, t);
        } return false;
    }
    ...
    private Boolean isQueryBookInfo(JSONObject bookForUser, int type) {
        ...
        int bookCollectedStatusType = type == 0 ? useUniversalCardBookCollectedStatusType.getValue() :
        ...
        int medalGenerationStatusType = type == 0 ? useUniversalCardMedalGenerationStatusType.getValue() :
    ...
    private JSONObject queryUniversalAnimal(String bookId, JSONObject animal) {
        try {
            ...
            for (int i = 0; i < animalForUserList.length(); i++) {
                jo = animalForUserList.getJSONObject(i); int star = jo.getInt("star");
                ...
                if (isbetteranimal) {
                    animal = jo.getJSONObject("animal"); animal.put("star", star); animal.put("count", count);animal.put("hasCollected", hasCollected);
                }
            }
        } catch (Throwable t) {
            Log.i(TAG, "queryUniversalAnimal err:"); Log.printStackTrace(TAG, t);
        } return animal;
    }
    ...
    private Boolean consumeProp(String propId, String propType) {
        try {
            ...
            jo = jo.getJSONObject("data"); String propName = jo.getJSONObject("propConfig").getString("propName");

            JSONObject animal = jo.getJSONObject("useResult").optJSONObject("animal");
            Log.forest("使用道具🎭[" + propName + "]" + getAnimalInfo(animal)); checkAnimalAndGiftToFriend(animal);
            return true;
        } catch (Throwable t) {
            Log.i(TAG, "consumeProp err:"); Log.printStackTrace(TAG, t);
        } return false;
    }
    ...
    private void collectToFriend() {
        try {
            JSONObject jo = new JSONObject(AntDodoRpcCall.queryFriend()); if (MessageUtil.checkResultCode(TAG, jo)) {
                int count = 0;
                ...
                for (int i = 0; i < limitList.length(); i++) {
                    ...
                    if (limit.getString("actionCode").equals("COLLECT_TO_FRIEND")) {
                        if (limit.getLong("startTime") > System.currentTimeMillis()) {
                            return;
                        } count = limit.getInt("leftLimit"); break;
                    }

                } JSONArray friendList = jo.getJSONObject("data").getJSONArray("friends");
                for (int i = 0; i < friendList.length() && count > 0; i++) {
                    JSONObject friend = friendList.getJSONObject(i); if (friend.getBoolean("dailyCollect")) {
                        continue;
                    ...
                    if (collectToFriendType.getValue() != CollectToFriendType.COLLECT) {
                        ...
                        continue;
                    } jo = new JSONObject(AntDodoRpcCall.collect(useId)); if (MessageUtil.checkResultCode(TAG, jo)) {
                        String userName = UserIdMap.getMaskName(useId);
                        JSONObject animal = jo.getJSONObject("data").optJSONObject("animal");
                        Log.forest("帮抽卡片🦕[" + userName + "]" + getAnimalInfo(animal)); count--;
                    }
                }

            }
        } catch (Throwable t) {
            Log.i(TAG, "collectHelpFriend err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private void generateBookMedal() {
        ...
        try {
            boolean hasMore; int pageStart = 0; do {
                JSONObject jo = new JSONObject(AntDodoRpcCall.queryBookList(9, pageStart));
                if (!MessageUtil.checkResultCode(TAG, jo)) {
                    break;
                } jo = jo.getJSONObject("data"); hasMore = jo.getBoolean("hasMore"); pageStart += 9;
                ...
                for (int i = 0; i < bookForUserList.length(); i++) {
                    ...
                    MedalGenerationStatus medalGenerationStatus = MedalGenerationStatus.valueOf(jo.optString(
                            "medalGenerationStatus"));
                    if (medalGenerationStatus == MedalGenerationStatus.CAN_GENERATE) {
                        ...
                        if (bookMedalOptions.getValue().contains("collectHistoryAnimal") && Objects.equals("END",
                                jo.optString("bookStatus"))) {
            ...
            Log.i(TAG, "generateBookMedal err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private void checkAnimalAndGiftToFriend(JSONObject animal) {
        if (animal == null || !giftToFriend.getValue() || useCollectTimingType.getValue() != TimingType.LAST_DAY) {
            return;
        } String targetUserId = getGiftToFriendTargetUserId(); if (targetUserId == null) {
            return;
        } try {
            if (!FantasticLevel.MAGIC.name().equals(animal.getString("fantasticLevel"))) {
                return;
            } String bookId = animal.getString("bookId"); JSONObject jo = new JSONObject(AntDodoRpcCall.homePage());
            if (!MessageUtil.checkResultCode(TAG, jo)) {
                return;
            } jo = jo.getJSONObject("data").getJSONObject("animalBook"); if (!bookId.equals(jo.getString("bookId"))) {
                return;
            } giftToFriend(animal, targetUserId);
        } catch (Throwable t) {
            Log.i(TAG, "checkAnimalAndGiftToFriend err:"); Log.printStackTrace(TAG, t);
        }
    }

    private String getGiftToFriendTargetUserId() {
        Set<String> set = giftToFriendList.getValue(); if (set.isEmpty()) {
            return null;
    ...
    private void giftToFriend(String targetUserId) {
        try {
            boolean hasMore; int pageStart = 0; do {
                ...
                JSONArray bookForUserList = jo.getJSONArray("bookForUserList");
                for (int i = 0; i < bookForUserList.length(); i++) {
                    jo = bookForUserList.getJSONObject(i); String collectProgress = jo.getString("collectProgress");
            ...
            Log.i(TAG, "giftToFriend err:"); Log.printStackTrace(TAG, t);
        }
    }
        ...
        try {
            ...
            for (int i = 0; i < animalForUserList.length(); i++) {
                ...
                if (animalForUser.optInt("star") < star) {
                    continue;
                } int count = animalForUser.getJSONObject("collectDetail").optInt("count"); if (count <= 0) {
                    continue;
                } JSONObject animal = animalForUser.getJSONObject("animal"); for (int j = 0; j < count; j++) {
            ...
            Log.i(TAG, "giftToFriend err:"); Log.printStackTrace(TAG, th);
        }
    }

    private Boolean giftToFriend(JSONObject animal, String targetUserId) {
        try {
            String animalId = animal.getString("animalId"); if (targetUserId.equals(UserIdMap.getCurrentUid())) {
                return false;
