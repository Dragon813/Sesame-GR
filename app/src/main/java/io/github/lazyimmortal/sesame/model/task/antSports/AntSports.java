...
public class AntSports extends ModelTask {
  ...
  public ModelFields getFields() {
    ...
    modelFields.addField(
        walkCustomPathIdList =
            new SelectModelField(
                "walkCustomPathIdList",
                "行走路线 | 自定义路线列表",
                new LinkedHashSet<>(),
                WalkPath::getList,
                "请选择要行走的路线，选择多条则随机走其中一条"));
    modelFields.addField(sportsTasks = new BooleanModelField("sportsTasks", "运动任务", false));
    modelFields.addField(
        receiveCoinAsset = new BooleanModelField("receiveCoinAsset", "收运动币", false));
    modelFields.addField(
        donateCharityCoinType =
            new ChoiceModelField(
                "donateCharityCoinType",
                "捐运动币 | 方式",
                DonateCharityCoinType.ZERO,
                DonateCharityCoinType.nickNames));
    modelFields.addField(
        donateCharityCoinAmount =
            new IntegerModelField("donateCharityCoinAmount", "捐运动币 | 数量(每次)", 100));
    modelFields.addField(
        coinExchangeDoubleCard =
            new BooleanModelField("coinExchangeDoubleCard", "运动币兑换限时能量双击卡", false));
    modelFields.addField(club = new BooleanModelField("club", "抢好友 | 开启", false));
    modelFields.addField(
        clubTrainItemType =
            new ChoiceModelField(
                "clubTrainItemType", "抢好友 | 训练动作", TrainItemType.NONE, TrainItemType.nickNames));
    modelFields.addField(
        clubTradeMemberType =
            new ChoiceModelField(
                "clubTradeMemberType",
                "抢好友 | 抢购动作",
                TradeMemberType.NONE,
                TradeMemberType.nickNames));
    modelFields.addField(
        clubTradeMemberList =
            new SelectModelField(
                "clubTradeMemberList", "抢好友 | 好友列表", new LinkedHashSet<>(), AlipayUser::getList));
    modelFields.addField(tiyubiz = new BooleanModelField("tiyubiz", "文体中心", false));
    modelFields.addField(
        minExchangeCount = new IntegerModelField("minExchangeCount", "行走捐 | 最小捐步步数", 0));
    modelFields.addField(
        latestExchangeTime =
            new IntegerModelField("latestExchangeTime", "行走捐 | 最晚捐步时间(24小时制)", 22));
  ...
  public void run() {
    try {
      ...
      if (donateCharityCoinType.getValue() != DonateCharityCoinType.ZERO) queryProjectList();

      if (coinExchangeDoubleCard.getValue()) {
        coinExchangeItem("AMS2024032927086104");
      }

      if (minExchangeCount.getValue() > 0) queryWalkStep();
      ...
      if (sportsTasks.getValue()) sportsTasks();
      ...
      for (int i = 0; i < taskList.length(); i++) {
        ...
        if (TaskStatus.WAIT_COMPLETE.name().equals(taskStatus)) {
          ...
          if (jo.optBoolean("multiTask")) {
            ...
            taskName =
                taskName.replaceAll("（.*/.*）", "(" + currentNum + "/" + limitConfigNum + ")");
          }
    ...
    try {
      ...
      int useStepCount =
          Math.min(
              Math.min(
                  remainStepCount,
                  hasTreasureBox() ? RandomUtil.nextInt(500, 1000) : remainStepCount),
  ...
  private void queryWalkStep() {
    ...
    try {
      ...
      JSONObject walkCharityActivityModel =
      ...
      jo =
          new JSONObject(
              AntSportsRpcCall.donateWalkExchange(activityId, produceQuantity, donateToken));
      if (!MessageUtil.checkResultCode(TAG, jo)) {
        return;
      }
    ...
    try {
      ...
      if (jo.optBoolean("success")) {
        ...
        for (int i = 0; i < userTaskList.length(); i++) {
          ...
          if (!"TODO".equals(jo.getString("status"))) continue;
      ...
      if (jo.optBoolean("success")) {
        ...
        if (jo.optBoolean("success")) {
          ...
          for (int i = 0; i < dataList.length(); i++) {
            ...
            jo =
                new JSONObject(
                    AntSportsRpcCall.participate(pointOptions, InstanceId, ResultId, roundId));
            if (jo.optBoolean("success")) {
              jo = jo.getJSONObject("data");
  ...
  private void userTaskRightsReceive() {
    try {
      ...
      if (jo.optBoolean("success")) {
        ...
        for (int i = 0; i < userTaskList.length(); i++) {
          jo = userTaskList.getJSONObject(i);
          if (!"COMPLETED".equals(jo.getString("status"))) continue;
          ...
          if (jo.optBoolean("success")) {
            ...
            for (int j = 0; j < rightsRuleList.length(); j++) {
              ...
              award
                  .append(jo.getString("rightsName"))
                  .append("*")
    ...
    try {
      ...
      if (jo.optBoolean("success")) {
        ...
        for (int i = 0; i < userPathGoRewardList.length(); i++) {
          ...
          if (!"UNRECEIVED".equals(jo.getString("status"))) continue;
  ...
  private void tiyubizGo(
      String countDate, String title, int goStepCount, String pathId, String userPathRecordId) {
    try {
    ...
    try {
      ...
      if (!trainInfo.getBoolean("training")) {
        ...
        if (queryTrainItemjo.has("bizId")) {
          ...
          JSONObject jo =
              new JSONObject(AntSportsRpcCall.trainMember(itemType, memberId, originBossId));
          if (!MessageUtil.checkResultCode(TAG, jo)) {
            return;
          }
          Log.other(
  ...
  private Boolean buyMember(String roomId, JSONObject member) {
    ...
    try {
      ...
      String currentBossShowName =
          UserIdMap.getShowName(currentBossId) != null
              ? UserIdMap.getShowName(currentBossId)
              : currentBossId;
      String memberId = member.getString("memberId");
      String originBossId = member.getString("originBossId");
      ...
      JSONObject jo =
          new JSONObject(
              AntSportsRpcCall.buyMember(currentBossId, memberId, originBossId, priceInfo, roomId));
      if (MessageUtil.checkResultCode(TAG, jo)) {
        String userName = UserIdMap.getShowName(originBossId);
