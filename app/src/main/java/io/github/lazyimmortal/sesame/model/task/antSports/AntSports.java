...
public class AntSports extends ModelTask {
    ...
    public ModelFields getFields() {
        ...
        modelFields.addField(walkPathTheme = new ChoiceModelField("walkPathTheme", "行走路线 | 路线主题",
                WalkPathTheme.DA_MEI_ZHONG_GUO, WalkPathTheme.nickNames));
        modelFields.addField(walkCustomPathIdList = new SelectModelField("walkCustomPathIdList", "行走路线 | 自定义路线列表",
                new LinkedHashSet<>(), WalkPath::getList, "请选择要行走的路线，选择多条则随机走其中一条"));
        ...
        modelFields.addField(donateCharityCoinType = new ChoiceModelField("donateCharityCoinType", "捐运动币 | 方式",
                DonateCharityCoinType.ZERO, DonateCharityCoinType.nickNames));
        modelFields.addField(donateCharityCoinAmount = new IntegerModelField("donateCharityCoinAmount",
                "捐运动币 | 数量" + "(每次)", 100));
        modelFields.addField(coinExchangeDoubleCard = new BooleanModelField("coinExchangeDoubleCard", "运动币兑换限时能量双击卡",
                false));
        modelFields.addField(club = new BooleanModelField("club", "抢好友 | 开启", false));
        modelFields.addField(clubTrainItemType = new ChoiceModelField("clubTrainItemType", "抢好友 | 训练动作",
                TrainItemType.NONE, TrainItemType.nickNames));
        modelFields.addField(clubTradeMemberType = new ChoiceModelField("clubTradeMemberType", "抢好友 | 抢购动作",
                TradeMemberType.NONE, TradeMemberType.nickNames));
        modelFields.addField(clubTradeMemberList = new SelectModelField("clubTradeMemberList", "抢好友 | 好友列表",
        ...
        modelFields.addField(latestExchangeTime = new IntegerModelField("latestExchangeTime", "行走捐 | 最晚捐步时间(24小时制)",
                22));
    ...
    public void boot(ClassLoader classLoader) {
        try {
            XposedHelpers.findAndHookMethod("com.alibaba.health.pedometer.core.datasource.PedometerAgent",
                    classLoader, "readDailyStep", new XC_MethodHook() {
                @Override
                protected void afterHookedMethod(MethodHookParam param) {
                    int originStep = (Integer) param.getResult(); int step = tmpStepCount();
            ...
            Log.i(TAG, "hook readDailyStep err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    public Boolean check() {
        if (TaskCommon.IS_ENERGY_TIME) {
            Log.other("任务暂停⏸️支付宝运动:当前为仅收能量时间"); return false;
        } return true;
    }
    ...
    public void run() {
        try {
            ...
            if (tiyubiz.getValue()) {
                userTaskGroupQuery("SPORTS_DAILY_SIGN_GROUP"); userTaskGroupQuery("SPORTS_DAILY_GROUP");
                userTaskRightsReceive(); pathFeatureQuery(); participate();
            }
            ...
            if (receiveCoinAsset.getValue()) {
                receiveCoinAsset(); AntSportsRpcCall.pickAllEnergyBall();
            }
        } catch (Throwable t) {
    ...
    public int tmpStepCount() {
        if (tmpStepCount >= 0) {
            return tmpStepCount;
        } tmpStepCount = syncStepCount.getValue(); if (tmpStepCount > 0) {
            tmpStepCount = RandomUtil.nextInt(tmpStepCount, tmpStepCount + 2000); if (tmpStepCount > 100000) {
                tmpStepCount = 100000;
            }
        } return tmpStepCount;
    }
    ...
    private void sportsTasks() {
        try {
            signInCoinTask(); JSONObject jo = new JSONObject(AntSportsRpcCall.queryCoinTaskPanel());
            if (!MessageUtil.checkSuccess(TAG, jo)) {
                return;
            } jo = jo.getJSONObject("data"); if (!jo.has("taskList")) {
                return;
            } JSONArray taskList = jo.getJSONArray("taskList"); for (int i = 0; i < taskList.length(); i++) {
                jo = taskList.getJSONObject(i);

                String taskStatus = jo.getString("taskStatus"); if (TaskStatus.HAS_RECEIVED.name().equals(taskStatus)) {
                    return;
                }

                String taskName = jo.getString("taskName"); if (TaskStatus.WAIT_RECEIVE.name().equals(taskStatus)) {
                    String assetId = jo.getString("assetId"); int prizeAmount = jo.getInt("prizeAmount");
                ...
                if (!jo.has("taskAction")) {
                    ...
                    String taskAction = jo.getString("taskAction"); String taskId = jo.getString("taskId");
                    if (jo.optBoolean("multiTask")) {
                        int currentNum = jo.getInt("currentNum") + 1; int limitConfigNum = jo.getInt("limitConfigNum");
            ...
            Log.i(TAG, "sportsTasks err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private Boolean signUpTask(String taskId) {
        try {
            ...
            Log.i(TAG, "signUpTask err:"); Log.printStackTrace(TAG, t);
        } return false;
    }
    ...
    private Boolean completeTask(String taskAction, String taskId, String taskName) {
        try {
            ...
            if (MessageUtil.checkSuccess(TAG, jo)) {
                Log.other("运动任务🧾完成[得运动币:" + taskName + "]"); return true;
            }
        } catch (Throwable t) {
            Log.i(TAG, "completeTask err:"); Log.printStackTrace(TAG, t);
        } return false;
    }
    ...
    private void signInCoinTask() {
        try {
            ...
            if (!MessageUtil.checkSuccess(TAG, jo)) {
                return;
            } JSONObject data = jo.getJSONObject("data"); if (!data.getBoolean("signed")) {
                JSONObject subscribeConfig; if (data.has("subscribeConfig")) {
            ...
            Log.i(TAG, "signInCoinTask err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private void receiveCoinAsset() {
        try {
            ...
            if (!MessageUtil.checkSuccess(TAG, jo)) {
                return;
            } JSONObject data = jo.getJSONObject("data"); if (!data.has("recBubbleList"))
                return; JSONArray ja = data.getJSONArray("recBubbleList"); for (int i = 0; i < ja.length(); i++) {
                jo = ja.getJSONObject(i); if (!data.has("assetId"))
                    return; String assetId = jo.getString("assetId"); int coinAmount = jo.getInt("coinAmount");
    ...
    private void walk() {
        String goingPathId = queryGoingPathId(); do {
            ...
            } TimeUtil.sleep(1000); if (isNeedJoinNewPath(goingPathId)) {
                String joinPathId = queryJoinPathId(); if (checkJoinPathId(joinPathId)) {
    ...
    private Boolean isNeedJoinNewPath(String goingPathId) {
        if (goingPathId.isEmpty()) {
            ...
            JSONObject jo = queryPath(goingPathId); jo = jo.getJSONObject("userPathStep");
            ...
            Log.i(TAG, "isNeedJoinNewPath err:"); Log.printStackTrace(TAG, t);
        } return false;
    }
    ...
    private Boolean hasTreasureBox() {
        if (Status.hasFlagToday("sport::treasureBoxLimit")) {
            ...
            if (!MessageUtil.checkResultCode(TAG, jo)) {
                return false;
            } JSONArray ja = jo.getJSONArray("userMailList"); int count = 0; for (int i = 0; i < ja.length(); i++) {
                jo = ja.getJSONObject(i); if (!"SPORTSPROD_GOPATH_AWARD_BOX".equals(jo.getString("templateId"))) {
                    continue;
            ...
            Log.i(TAG, "hasTreasureBox err:"); Log.printStackTrace(TAG, t);
        } return false;
    }
    ...
    private Boolean walkGo(JSONObject pathData) {
        try {
            ...
            int minGoStepCount = path.getInt("minGoStepCount"); int pathStepCount = path.getInt("pathStepCount");
            ...
            if (useStepCount < minGoStepCount || dayLimit) {
                return false;
            } String pathId = path.getString("pathId"); String pathName = path.getString("name");
            return walkGo(pathName, pathId, useStepCount);
        } catch (Throwable t) {
            Log.i(TAG, "walkGo err:"); Log.printStackTrace(TAG, t);
        } return false;
    }
    ...
    private Boolean walkGo(String pathName, String pathId, int useStepCount) {
        boolean result = false; try {
            ...
            if (MessageUtil.checkSuccess(TAG, jo)) {
                result = true; Log.other("行走路线🚶🏻‍♂️行走[" + pathName + "]#前进了" + useStepCount + "步");
                jo = jo.getJSONObject("data"); if (jo.has("completeInfo")) {
    ...
    private JSONObject queryWorldMap(String themeId) {
        JSONObject theme = null; try {
            JSONObject jo = new JSONObject(AntSportsRpcCall.queryWorldMap(themeId));
            if (MessageUtil.checkSuccess(TAG, jo)) {
                theme = jo.getJSONObject("data");
            }
        } catch (Throwable t) {
            Log.i(TAG, "queryWorldMap err:"); Log.printStackTrace(TAG, t);
        } return theme;
    }

    private JSONObject queryCityPath(String cityId) {
        JSONObject city = null; try {
            JSONObject jo = new JSONObject(AntSportsRpcCall.queryCityPath(cityId));
            if (MessageUtil.checkSuccess(TAG, jo)) {
                city = jo.getJSONObject("data");
            }
        } catch (Throwable t) {
            Log.i(TAG, "queryCityPath err:"); Log.printStackTrace(TAG, t);
        } return city;
    }

    private static JSONObject queryPath(String pathId) {
        JSONObject path = null; try {
            String date = Log.getFormatDate(); JSONObject jo = new JSONObject(AntSportsRpcCall.queryPath(date, pathId));
            if (MessageUtil.checkSuccess(TAG, jo)) {
                path = jo.getJSONObject("data"); parseRewardsByJSONObjectData(path);
            }
        } catch (Throwable t) {
            Log.i(TAG, "queryPath err:"); Log.printStackTrace(TAG, t);
        } return path;
    }
    ...
    private static void openTreasureBox(JSONArray treasureBoxList) {
        try {
            for (int i = 0; i < treasureBoxList.length(); i++) {
                JSONObject treasureBox = treasureBoxList.getJSONObject(i); receiveEvent(treasureBox.getString("boxNo"));
                TimeUtil.sleep(1000);
            }
        } catch (Throwable t) {
            Log.i(TAG, "openTreasureBox err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private static void receiveEvent(String eventBillNo) {
        try {
            ...
            if (MessageUtil.checkSuccess(TAG, jo)) {
                jo = jo.getJSONObject("data"); parseRewardsByJSONArrayRewards(jo.getJSONArray("rewards"), 0);
            }
        } catch (Throwable t) {
            Log.i(TAG, "receiveEvent err:"); Log.printStackTrace(TAG, t);
        }
    }

    private static void parseRewardsByJSONArrayRewards(JSONArray rewards, int rewardsType) {
        String rewardsTypeName; switch (rewardsType) {
            case 0:
                rewardsTypeName = "宝箱奖励"; break;
            case 1:
                rewardsTypeName = "中奖奖励"; break;
            case 2:
                rewardsTypeName = "终点奖励"; break;
            default:
                rewardsTypeName = "未知奖励"; break;
        } try {
            for (int i = 0; i < rewards.length(); i++) {
                ...
                Log.other("行走路线🚶🏻‍♂️收获" + rewardsTypeName + "[" + jo.getString("rewardName") + "*" + jo.getInt(
                        "count") + "]");
            }
        } catch (Throwable t) {
            Log.i(TAG, "parseRewardsByJSONArrayRewards err:"); Log.printStackTrace(TAG, t);
        }
    }

    private static void parseRewardsByJSONObjectData(JSONObject data) {
        try {
            JSONArray treasureBoxList = data.getJSONArray("treasureBoxList"); openTreasureBox(treasureBoxList);
            ...
            Log.i(TAG, "parseRewardsByJSONObjectData err:"); Log.printStackTrace(TAG, t);
        }
    }

    private String queryGoingPathId() {
        String goingPathId = ""; try {
            String date = Log.getFormatDate(); JSONObject jo = new JSONObject(AntSportsRpcCall.queryPath(date, ""));
            if (MessageUtil.checkSuccess(TAG, jo)) {
                jo = jo.getJSONObject("data"); goingPathId = jo.optString("goingPathId");
            }
        } catch (Throwable t) {
            Log.i(TAG, "queryGoingPathId err:"); Log.printStackTrace(TAG, t);
        } return goingPathId;
    }

    private String queryJoinPathId() {
        String pathId = TokenConfig.getCustomWalkPathId(walkCustomPathIdList.getValue()); if (pathId != null) {
            return pathId;
        }

        try {
            String themeId = WalkPathTheme.walkPathThemeIds[walkPathTheme.getValue()];
            JSONObject theme = queryWorldMap(themeId); if (theme == null) {
                return pathId;
            } JSONArray cityList = theme.getJSONArray("cityList"); for (int i = 0; i < cityList.length(); i++) {
                String cityId = cityList.getJSONObject(i).getString("cityId"); JSONObject city = queryCityPath(cityId);
                if (city == null) {
                    continue;
                } JSONArray cityPathList = city.getJSONArray("cityPathList");
                for (int j = 0; j < cityPathList.length(); j++) {
                    JSONObject cityPath = cityPathList.getJSONObject(j); pathId = cityPath.getString("pathId");
            ...
            Log.i(TAG, "queryJoinPathId err:"); Log.printStackTrace(TAG, t);
        } return pathId;
    }

    public static Boolean checkJoinPathId(String joinPathId) {
        try {
            JSONObject jo = queryPath(joinPathId); String goingPathId = jo.optString("goingPathId");
            if (Objects.equals(goingPathId, joinPathId)) {
                return false;
            } jo = jo.getJSONObject("userPathStep"); return !jo.optBoolean("dayLimit");
        } catch (Throwable t) {
            Log.i(TAG, "checkJoinPathId err:"); Log.printStackTrace(TAG, t);
        } return false;
    }
    ...
    public static Boolean joinPath(String pathId) {
        if (pathId == null) {
            ...
            JSONObject jo = new JSONObject(AntSportsRpcCall.joinPath(pathId)); if (MessageUtil.checkSuccess(TAG, jo)) {
            ...
            Log.i(TAG, "joinPath err:"); Log.printStackTrace(TAG, t);
        } return false;
    }
    ...
    private Boolean canDonateCharityCoinToday() {
        if (Status.hasFlagToday("sport::donateCharityCoin")) {
            ...
            if (footballFieldLongModel.length() == 0) {
                return true;
            } jo = footballFieldLongModel.getJSONObject(0); jo = jo.getJSONObject("personStatModel");
            ...
            Log.i(TAG, "canDonateCharityCoinToday err:"); Log.printStackTrace(TAG, t);
        } return false;
    }
    ...
    private void queryProjectList() {
        if (!canDonateCharityCoinToday()) {
            ...
            int donateCharityCoin = donateCharityCoinAmount.getValue(); if (charityCoinCount < donateCharityCoin) {
                return;
            } JSONArray ja = jo.getJSONObject("projectPage").getJSONArray("data");
            for (int i = 0; i < ja.length(); i++) {
                jo = ja.getJSONObject(i).getJSONObject("basicModel"); if (jo.optInt("acwProjectStatus") == 0) {
            ...
            Log.i(TAG, "queryProjectList err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private Boolean donate(int donateCharityCoin, String projectId, String title) {
        try {
            ...
            if (MessageUtil.checkResultCode(TAG, jo)) {
                Log.other("公益捐赠❤️[捐赠运动币:" + title + "]捐赠" + donateCharityCoin + "运动币"); return true;
            }
        } catch (Throwable t) {
            Log.i(TAG, "donate err:"); Log.printStackTrace(TAG, t);
        } return false;
    }
    ...
    private Boolean canDonateWalkExchangeToday() {
        if (Status.hasFlagToday("sport::donateWalk")) {
            ...
            if (userExchangeRecords.length() == 0) {
                return true;
            } jo = userExchangeRecords.getJSONObject(0); long gmtCreate = jo.getLong("gmtCreate");
            if (TimeUtil.isLessThanNowOfDays(gmtCreate)) {
                return true;
            } Status.flagToday("sport::donateWalk");
        } catch (Throwable t) {
            Log.i(TAG, "canDonateWalkExchangeToday err:"); Log.printStackTrace(TAG, t);
        } return false;
    }
    ...
    private void queryWalkStep() {
        if (!canDonateWalkExchangeToday()) {
            ...
            if (!MessageUtil.checkResultCode(TAG, jo)) {
                return;
            } jo = jo.getJSONObject("dailyStepModel"); int produceQuantity = jo.getInt("produceQuantity");
            ...
            Log.i(TAG, "queryWalkStep err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private void userTaskGroupQuery(String groupId) {
        try {
            String s = AntSportsRpcCall.userTaskGroupQuery(groupId); JSONObject jo = new JSONObject(s);
            if (jo.optBoolean("success")) {
                jo = jo.getJSONObject("group"); JSONArray userTaskList = jo.getJSONArray("userTaskList");
                for (int i = 0; i < userTaskList.length(); i++) {
                    jo = userTaskList.getJSONObject(i); if (!"TODO".equals(jo.getString("status")))
                        continue; JSONObject taskInfo = jo.getJSONObject("taskInfo");
                    String bizType = taskInfo.getString("bizType"); String taskId = taskInfo.getString("taskId");
            ...
            Log.i(TAG, "userTaskGroupQuery err:"); Log.printStackTrace(TAG, t);
        }
    }

    private void participate() {
        try {
            String s = AntSportsRpcCall.queryAccount(); JSONObject jo = new JSONObject(s);
            if (jo.optBoolean("success")) {
                double balance = jo.getDouble("balance"); if (balance < 100)
                    return; jo = new JSONObject(AntSportsRpcCall.queryRoundList()); if (jo.optBoolean("success")) {
                    JSONArray dataList = jo.getJSONArray("dataList"); for (int i = 0; i < dataList.length(); i++) {
                        jo = dataList.getJSONObject(i); if (!"P".equals(jo.getString("status")))
                            continue; if (jo.has("userRecord"))
                            continue; JSONArray instanceList = jo.getJSONArray("instanceList"); int pointOptions = 0;
                        String roundId = jo.getString("id"); String InstanceId = null; String ResultId = null;
                        for (int j = instanceList.length() - 1; j >= 0; j--) {
                            jo = instanceList.getJSONObject(j); if (jo.getInt("pointOptions") < pointOptions)
                                continue; pointOptions = jo.getInt("pointOptions"); InstanceId = jo.getString("id");
                        ...
                        if (jo.optBoolean("success")) {
                            jo = jo.getJSONObject("data"); String roundDescription = jo.getString("roundDescription");
            ...
            Log.i(TAG, "participate err:"); Log.printStackTrace(TAG, t);
        }
    }

    private void userTaskRightsReceive() {
        try {
            String s = AntSportsRpcCall.userTaskGroupQuery("SPORTS_DAILY_GROUP"); JSONObject jo = new JSONObject(s);
            if (jo.optBoolean("success")) {
                jo = jo.getJSONObject("group"); JSONArray userTaskList = jo.getJSONArray("userTaskList");
                for (int i = 0; i < userTaskList.length(); i++) {
                    jo = userTaskList.getJSONObject(i); if (!"COMPLETED".equals(jo.getString("status")))
                        continue; String userTaskId = jo.getString("userTaskId");
                    JSONObject taskInfo = jo.getJSONObject("taskInfo"); String taskId = taskInfo.getString("taskId");
                    ...
                    if (jo.optBoolean("success")) {
                        ...
                        StringBuilder award = new StringBuilder(); for (int j = 0; j < rightsRuleList.length(); j++) {
                        ...
                        Log.record("文体中心领取奖励"); Log.i(jo.toString());
                    }
                }
            } else {
                Log.record("文体中心领取奖励"); Log.i(s);
            }
        } catch (Throwable t) {
            Log.i(TAG, "userTaskRightsReceive err:"); Log.printStackTrace(TAG, t);
        }
    }

    private void pathFeatureQuery() {
        try {
            String s = AntSportsRpcCall.pathFeatureQuery(); JSONObject jo = new JSONObject(s);
            if (jo.optBoolean("success")) {
                JSONObject path = jo.getJSONObject("path"); String pathId = path.getString("pathId");
                String title = path.getString("title"); int minGoStepCount = path.getInt("minGoStepCount");
                if (jo.has("userPath")) {
                    ...
                    if ("COMPLETED".equals(userPathRecordStatus)) {
                        pathMapHomepage(pathId); pathMapJoin(title, pathId);
                    } else if ("GOING".equals(userPathRecordStatus)) {
                        pathMapHomepage(pathId); String countDate = Log.getFormatDate();
                        jo = new JSONObject(AntSportsRpcCall.stepQuery(countDate, pathId));
                        if (jo.optBoolean("success")) {
                            int canGoStepCount = jo.getInt("canGoStepCount"); if (canGoStepCount >= minGoStepCount) {
            ...
            Log.i(TAG, "pathFeatureQuery err:"); Log.printStackTrace(TAG, t);
        }
    }

    private void pathMapHomepage(String pathId) {
        try {
            String s = AntSportsRpcCall.pathMapHomepage(pathId); JSONObject jo = new JSONObject(s);
            if (jo.optBoolean("success")) {
                if (!jo.has("userPathGoRewardList"))
                    return; JSONArray userPathGoRewardList = jo.getJSONArray("userPathGoRewardList");
                for (int i = 0; i < userPathGoRewardList.length(); i++) {
                    jo = userPathGoRewardList.getJSONObject(i); if (!"UNRECEIVED".equals(jo.getString("status")))
                        continue; String userPathRewardId = jo.getString("userPathRewardId");
                    ...
                    if (jo.optBoolean("success")) {
                        ...
                        Log.record("文体中心开宝箱"); Log.i(jo.toString());
                    }
                }
            } else {
                Log.record("文体中心开宝箱"); Log.i(s);
            }
        } catch (Throwable t) {
            Log.i(TAG, "pathMapHomepage err:"); Log.printStackTrace(TAG, t);
        }
    }

    private void pathMapJoin(String title, String pathId) {
        try {
            JSONObject jo = new JSONObject(AntSportsRpcCall.pathMapJoin(pathId)); if (jo.optBoolean("success")) {
                Log.other("加入线路🚶🏻‍♂️[" + title + "]"); pathFeatureQuery();
            } else {
                Log.i(TAG, jo.toString());
            }
        } catch (Throwable t) {
            Log.i(TAG, "pathMapJoin err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private void tiyubizGo(String countDate, String title, int goStepCount, String pathId, String userPathRecordId) {
        try {
            ...
            JSONObject jo = new JSONObject(s); if (jo.optBoolean("success")) {
                jo = jo.getJSONObject("userPath");
                Log.other("行走线路🚶🏻‍♂️[" + title + "]#前进了" + jo.getInt("userPathRecordForwardStepCount") + "步");
                pathMapHomepage(pathId); boolean completed = "COMPLETED".equals(jo.getString("userPathRecordStatus"));
                if (completed) {
                    Log.other("完成线路🚶🏻‍♂️[" + title + "]"); pathFeatureQuery();
                }
            } else {
                Log.i(TAG, s);
            }
        } catch (Throwable t) {
            Log.i(TAG, "tiyubizGo err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private void queryClubHome() {
        try {
            ...
            if (!MessageUtil.checkResultCode(TAG, joBubble)) {
                return;
            } JSONObject mainRoom = joBubble.getJSONObject("mainRoom"); if (mainRoom.has("bubbleList")) {
                ...
                for (int k = 0; k < bubbleList.length(); k++) {
                    ...
                    collectBubble(bubbleId, "[买卖]"); TimeUtil.sleep(200);
                }
            }
            ...
            if (!MessageUtil.checkResultCode(TAG, jo)) {
                return;
            } JSONArray roomList = jo.getJSONArray("roomList"); for (int i = 0; i < roomList.length(); i++) {
                // 检查可以购买好友的房号i
                JSONObject room = roomList.getJSONObject(i); String roomId = room.getString("roomId");
                ...
                if (room.has("bubbleList")) {
                    ...
                    for (int l = 0; l < roombubbleList.length(); l++) {
                        ...
                        collectBubble(bubbleId, "[训练]"); TimeUtil.sleep(200);
                    }
                }
                ...
                if (clubTradeMemberType.getValue() != TradeMemberType.NONE) {
                    queryMemberPriceRanking(roomId); TimeUtil.sleep(200);
                }
            } TimeUtil.sleep(200);
            ...
            for (int j = 0; j < roomListTrain.length(); j++) {
                ...
                if (roomTrain.getJSONArray("memberList").length() != 0) {
                    JSONObject member = roomTrain.getJSONArray("memberList").getJSONObject(0); trainMember(member);
                    TimeUtil.sleep(200);
                }
            }
        } catch (Throwable t) {
            Log.i(TAG, "queryClubHome err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private void trainMember(JSONObject member) {
        try {
            String roomId = member.getString("roomId"); String memberId = member.getString("memberId");
            String originBossId = member.getString("originBossId");
            JSONObject trainInfo = member.getJSONObject("trainInfo");

            String userName = UserIdMap.getShowName(originBossId); if (!trainInfo.getBoolean("training")) {
                ...
                if (queryTrainItemjo.has("bizId")) {
                    String bizId = queryTrainItemjo.getString("bizId"); String taskAction = "SHOW_AD";
                    queryTrainItemjo = queryTrainItemjo.getJSONObject("taskDetail");
                    String taskId = queryTrainItemjo.getString("taskId");
                    JSONObject jo = new JSONObject(AntSportsRpcCall.DoubletrainMember(itemType, bizId, memberId,
                            originBossId));
            ...
            Log.i(TAG, "trainMember err:"); Log.printStackTrace(TAG, t);
        }
    }

    // 抢好友大战-蹲点训练
    private void autoTrainMember(String roomId, Long gmtEnd) {
        String taskId = "TRAIN|" + roomId; if (!hasChildTask(taskId)) {
    ...
    private void queryMemberPriceRanking(String roomId) {
        int energyBalance; try {
            ...
            energyBalance = assetsInfo.getInt("energyBalance"); TimeUtil.sleep(200);
            JSONObject jo = new JSONObject(AntSportsRpcCall.queryMemberPriceRankingEnergy(energyBalance));
            if (!MessageUtil.checkResultCode(TAG, jo)) {
                return;
            } energyBalance = jo.getInt("energyBalance"); jo = jo.getJSONObject("rank");
            JSONArray ja = jo.getJSONArray("data"); for (int i = 0; i < ja.length(); i++) {
                jo = ja.getJSONObject(i); int price = jo.getInt("price"); if (price > energyBalance) {
                    continue;
                ...
                boolean canbuyMember = true; JSONObject joTrain = new JSONObject(AntSportsRpcCall.queryClubHome());
                if (!MessageUtil.checkResultCode(TAG, joTrain)) {
                    return;
                } JSONArray roomListTrain = joTrain.getJSONArray("roomList");
                for (int j = 0; j < roomListTrain.length(); j++) {
                ...
                if (canbuyMember) {
                    buyMember(roomId, queryClubMember(jo)); return;
                }
            }
        } catch (Throwable t) {
            Log.i(TAG, "queryMemberPriceRanking err:"); Log.printStackTrace(TAG, t);
        } return;
    }

    private JSONObject queryClubMember(JSONObject member) {
        try {
            String memberId = member.getString("memberId"); String originBossId = member.getString("originBossId");
            ...
            Log.i(TAG, "queryClubMember err:"); Log.printStackTrace(TAG, t);
        } return null;
    }
    ...
    private Boolean buyMember(String roomId, JSONObject member) {
        if (member == null) {
            ...
            String currentBossShowName = UserIdMap.getShowName(currentBossId) != null ?
                    UserIdMap.getShowName(currentBossId) : currentBossId;
            String memberId = member.getString("memberId"); String originBossId = member.getString("originBossId");
            JSONObject priceInfo = member.getJSONObject("priceInfo");
            JSONObject jo = new JSONObject(AntSportsRpcCall.buyMember(currentBossId, memberId, originBossId,
                    priceInfo, roomId));
            if (MessageUtil.checkResultCode(TAG, jo)) {
            ...
            Log.i(TAG, "buyMember err:"); Log.printStackTrace(TAG, t);
        } return false;
    }
    ...
    private void coinExchangeItem(String itemId) {
        try {
            ...
            if (!MessageUtil.checkSuccess(TAG, jo)) {
                return;
            } jo = jo.getJSONObject("data"); if (!"OK".equals(jo.optString("exchangeBtnStatus"))) {
                return;
            } jo = jo.getJSONObject("itemBaseInfo"); String itemTitle = jo.getString("itemTitle");
            int valueCoinCount = jo.getInt("valueCoinCount");
            jo = new JSONObject(AntSportsRpcCall.exchangeItem(itemId, valueCoinCount));
            if (!MessageUtil.checkSuccess(TAG, jo)) {
                return;
            } jo = jo.getJSONObject("data"); if (jo.optBoolean("exgSuccess")) {
    ...
    public interface WalkPathTheme {
        ...
        String[] walkPathThemeIds = {"M202308082226", "M202401042147", "V202405271625", "202404221422",
                "WF202312050200", "V202409061650"};
    }
