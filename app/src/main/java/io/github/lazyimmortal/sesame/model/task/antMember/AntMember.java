...
public class AntMember extends ModelTask {
    ...
    public Boolean check() {
        if (TaskCommon.IS_ENERGY_TIME) {
            Log.other("任务暂停⏸️蚂蚁会员:当前为仅收能量时间"); return false;
        } return true;
    }
    ...
    private void memberSign() {
        try {
            if (!Status.hasFlagToday("member::sign")) {
                ...
                TimeUtil.sleep(500); if (MessageUtil.checkResultCode(TAG, jo)) {
            ...
            Log.i(TAG, "memberSign err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private static void queryPointCert(int page, int pageSize) {
        try {
            ...
            for (int i = 0; i < jaCertList.length(); i++) {
                jo = jaCertList.getJSONObject(i); String bizTitle = jo.getString("bizTitle");
                String id = jo.getString("id"); int pointAmount = jo.getInt("pointAmount");
            ...
            Log.i(TAG, "queryPointCert err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private void signPageTaskList() {
        try {
            do {
                ...
                TimeUtil.sleep(500); boolean doubleCheck = false;
                ...
                for (int i = 0; i < categoryTaskList.length(); i++) {
                    ...
                    String type = jo.getString("type"); if (Objects.equals("BROWSE", type)) {
            ...
            Log.i(TAG, "signPageTaskList err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private void queryAllStatusTaskList() {
        try {
            ...
            Log.i(TAG, "queryAllStatusTaskList err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private void promise() {
        try {
            ...
            Log.i(TAG, "promise err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private JSONObject querySingleTemplate(String templateId) {
        try {
            ...
            if (!MessageUtil.checkResultCode(TAG, jo)) {
                return null;
            } jo = jo.getJSONObject("data"); JSONObject result = new JSONObject();
            ...
            joinRule.remove("joinRule"); result.put("joinRule", joinRule);
            ...
            dataSourceRule.remove("canSelectValues"); result.put("dataSourceRule", dataSourceRule);
            return result;
        } catch (Throwable t) {
            Log.i(TAG, "querySingleTemplate err:"); Log.printStackTrace(TAG, t);
        } return null;
    }
    ...
    private void promiseJoin(JSONObject data) {
        if (data == null) {
            ...
            if (!MessageUtil.checkResultCode(TAG, jo)) {
                return;
            } jo = jo.getJSONObject("data"); String promiseName = jo.getString("promiseName");
            Log.other("生活记录📝加入[" + promiseName + "]");
        } catch (Throwable t) {
            Log.i(TAG, "promiseJoin err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private static Boolean doBrowseTask(JSONArray taskList) {
        boolean doubleCheck = false; try {
            for (int i = 0; i < taskList.length(); i++) {
                JSONObject task = taskList.getJSONObject(i); if (task.getBoolean("hybrid")) {
            ...
            Log.i(TAG, "doBrowseTask err:"); Log.printStackTrace(TAG, t);
        } return doubleCheck;
    }
    ...
    private static Boolean doBrowseTask(JSONObject task, int left, int right) {
        boolean doubleCheck = false; try {
            ...
            String name = taskConfigInfo.getString("name"); Long id = taskConfigInfo.getLong("id");
            ...
            for (int i = left; i <= right; i++) {
                ...
                TimeUtil.sleep(300); if (!MessageUtil.checkResultCode(TAG, jo)) {
                    continue;
                } String[] targetBusinessArray = targetBusiness.split("#"); String bizParam;
                String bizSubType; if (targetBusinessArray.length > 2) {
                    bizParam = targetBusinessArray[2]; bizSubType = targetBusinessArray[1];
                } else {
                    bizParam = targetBusinessArray[1]; bizSubType = targetBusinessArray[0];
    ...
    private void goldBillCollect(String signInfo) {
        try {
            ...
            JSONObject jsonObject = new JSONObject(str); if (!jsonObject.optBoolean("success")) {
            ...
            int length = jsonArray.length(); if (length == 0) {
                return;
            ...
            Log.i(TAG, "signIn err:"); Log.printStackTrace(TAG, th);
        }
    }
        ...
        try {
            try {
                ...
                if (String.valueOf(true).equals(str)) {
                    return;
                } str = AntMemberRpcCall.continueSignIn(); TimeUtil.sleep(300);
                jsonObject = new JSONObject(str); if (!jsonObject.optBoolean("success")) {
                ...
                if (jsonArray == null || jsonArray.length() == 0) {
                    return;
                } str = AntMemberRpcCall.batchReceivePointBall(); TimeUtil.sleep(300);
                jsonObject = new JSONObject(str); if (jsonObject.optBoolean("success")) {
                ...
                Log.i(TAG, "batchReceive err:"); Log.printStackTrace(TAG, th);
            }
        } catch (Throwable t) {
            Log.printStackTrace(TAG, t);
        }
    ...
    private void collectSesame() {
        try {
            ...
            if (!entrance.optBoolean("openApp")) {
                Log.other("芝麻信用💌未开通"); return;
            } jo = new JSONObject(AntMemberRpcCall.queryCreditFeedback()); TimeUtil.sleep(300);
            if (!MessageUtil.checkResultCode(TAG, jo)) {
                return;
            } JSONArray ja = jo.getJSONArray("creditFeedbackVOS");
            for (int i = 0; i < ja.length(); i++) {
                jo = ja.getJSONObject(i); if (!"UNCLAIMED".equals(jo.getString("status"))) {
                    continue;
    ...
    private void memberPointExchangeBenefit() {
        try {
            ...
            if (!MessageUtil.checkResultCode(TAG, jo)) {
                ...
                Log.record("会员积分[未实名账号无可兑换权益]"); return;
            ...
            Log.i(TAG, "memberPointExchangeBenefit err:"); Log.printStackTrace(TAG, t);
        }
    }
    ...
    private Boolean exchangeBenefit(String benefitId, String itemId) {
        try {
            ...
            if (MessageUtil.checkResultCode(TAG, jo)) {
                Status.memberPointExchangeBenefitToday(benefitId); return true;
            }
        } catch (Throwable t) {
            Log.i(TAG, "exchangeBenefit err:"); Log.printStackTrace(TAG, t);
        } return false;
    }
    ...
    private void RecommendTask() {
        try {
            ...
            for (int i = 0; i < taskDetailList.length(); i++) {
                ...
                boolean canAccess = taskDetail.optBoolean("canAccess", false); if (!canAccess) {
                ...
                // 检查 success 字段
                boolean success = triggerResult.getBoolean("success"); if (success) {
            ...
            Log.i(TAG, "RecommendTask err:"); Log.printStackTrace(TAG, th);
        }
    }
    ...
    private void OrdinaryTask() {
        try {
            ...
            Log.i(TAG, "OrdinaryTask err:"); Log.printStackTrace(TAG, th);
        }
    }
    ...
    private void signinCalendar() {
        try {
            ...
            if (!MessageUtil.checkSuccess(TAG, jo)) {
                return;
            } boolean signed = jo.optBoolean("isSignInToday"); if (!signed) {
            ...
            Log.i(TAG, "signinCalendar err:"); Log.printStackTrace(TAG, t);
        }
    }
}
